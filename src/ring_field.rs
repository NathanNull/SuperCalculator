...
try_ops_trait!((pub) trait RingOps {
    fn try_ring_ops(&self, rhs: &dyn Value, op: Op) -> Option<Box<dyn Value>> {
        if ([T]: Self = T, T: Ring + Value) {
            |lhs: &T, rhs: &dyn Value, op| {
        ...
        } else ([T]: Self = T, Self: Value) {
            |_,_,_| None
        }
    }
});

try_ops_trait!((pub) trait FieldOps {
    fn try_field_ops(&self, rhs: &dyn Value, op: Op) -> Option<Box<dyn Value>> {
        if ([T]: Self = T, Self: Field + Value) {
            |lhs: &Self, rhs, op| {
                if let Some(res) = lhs.try_ring_ops(rhs, op) {
                    return Some(res)
                }
                if rhs.get_type() == lhs.get_type() {
                    let rhs = rhs.downcast::<T>().expect("Downcast error");
